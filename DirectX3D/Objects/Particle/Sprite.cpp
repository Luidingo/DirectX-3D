...
void Sprite::Update()
{
	if (!isActive) return; // 비활성화 시 종료

	time += speed * DELTA; // 경과 시간 * 재생 속도 만큼 누적 재생 시간 증가

	if (time >= interval) // 재생 시간이 간격을 넘어서면
	{
		curFrameCount++; // 프레임 +1
...
void Sprite::Create()
{
	...
	vertexBuffer = new VertexBuffer(&vertex, sizeof(VertexUV), 1); // 버퍼 용량 = 1
}
