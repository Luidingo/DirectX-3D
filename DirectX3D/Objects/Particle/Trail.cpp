...
{
	material->SetShader(L"Basic/Texture.hlsl"); // 기본만 읽어도 텍스처는 적용됨
												// -> 빌보드 옵션 등을 활용할 수는 없음
												// -> 해당 클래스에서는 어차피 사용할 일 없음
	...
	rasterizerState[1]->CullMode(D3D11_CULL_NONE); // 앞과 뒤 모두 출력하기
...
Trail::~Trail()
{
	...
	// start, end는 다른 곳에서 만든 것을 받으므로 여기서 삭제하지 않음
}

void Trail::Update()
{
	//if (!Active()) return; // <- 비활성화 시 업데이트 안 함
	// 위 코드를 끄면 궤적이 안 보이는 중이더라도 항상 정점 갱신을 진행할 것
	// -> 최적화 문제가 발생하지만, 궤적이 항상 모델과 일치하는 효과를 낼 수 있다
	...
	for (UINT i = 0; i <= width; ++i)
	{
		...
		{
			startDestPos = vertices[(i - 1) * 2 + 0].pos;	// 현재 위치 왼쪽 정점의 위치 
															// -> 바로 직전 정점이 방금까지 있었던 곳
			endDestPos = vertices[(i - 1) * 2 + 1].pos;
		}

		// 도출된 목적지와 현재 위치를 시간 경과(+ 속도)에 따라 보간 수행
...
void Trail::CreateMesh()
{
	...
	for (UINT i = 0; i <= width; ++i)	// i가 width에 도달해도 한 번 더 진행
	{
		// 벡터에 넣을 정점 준비
		VertexUV vertex;
		...
		vertex.uv = { (float)i / width, 1 };	// 잘린 구간을 u, 아래를 v로 하는 UV로 지정 후
		vertices.push_back(vertex);				// 정점을 벡터에 추가
	...
	for (int i = 0; i < width; ++i) // 가로 끝까지 가지 않고 i < width로 반복 조건 마무리 
	{
		...
		// 0 - 1 - 2, 2 - 1 - 3 : 앞면 텍스처 그리기 조합
		// 0 - 2 - 1, 1 - 2 - 3 : 뒷면 텍스처 그리기 조합
		// * 원래는 쓰면 안 되지만, 현재는 컬링 모드가 앞뒤 구분 없이 모두 출력하므로 가능함
	}
