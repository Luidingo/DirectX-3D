...
{
private:
	// 스파크가 가져야 할 데이터의 내부 집합 : 각 입자의 데이터
...
private:
	float minRadius = 5.f;				// 최소 반경
	float maxRadius = 10.f;				// 최대 반경
	
	float minSize = 0.1f;				// 최소 크기
	float maxSize = 1.0f;				// 최대 크기

	FloatValueBuffer* buffer;
	ColorBuffer* startColorBuffer;		// 시작 시 색깔
	ColorBuffer* endColorBuffer;		// 종료 시 색깔

	// -> 파티클로 만들어지는 대상들의 특징 : 옵션이 하나로 정해지지 않고 범위 내에서 랜덤으로 취급되는 경우가 많다
