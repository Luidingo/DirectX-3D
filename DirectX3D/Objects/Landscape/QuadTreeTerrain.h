...
{
	// 쿼드 트리 터레인 : 쿼드 트리 자료 구조를 적용한 터레인 클래스

private:
	float MAX_HEIGHT = 20;
	UINT MIN_TRIANGLE = 2048;				// 자료 구조를 나눌 때, 각 단위가 성립하기 위한 최소 삼각형 개수
	...
	struct Node								// 지형이 쿼드 트리에 의해 나뉘는 분할 단위 (A*에 쓰이는 것과는 다름)
	{
		// 분할 단위에서 필요한 데이터
		float x, z, size;					// 노드의 위치, 노드의 크기
		UINT triangleCount;					// 이 노드가 갖고 있는 삼각형(= 렌더할 대상)의 개수

		Mesh<VertexType>* mesh = nullptr;	// 이 지형이 갖고 있는(전체 지형의 부분적인) 형태
...
private:
	void RenderNode(Node* node);			// 지역을 나누어서 각각 따로 렌더
