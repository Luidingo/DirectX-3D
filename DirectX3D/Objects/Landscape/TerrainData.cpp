...
void TerrainData::AlignVertexData()
{
	...
	alignedVertices.reserve(indices.size());	// 렌더용 목록을 만드는 과정에서 연관 정점을 쓴다는 공통점이 있어서
												// 순번 목록의 크기 = 정리된 정점의 벡터 크기로 이해해도 됨
	...
	for (UINT z = 0; z < height - 1; ++z)		// 위치에 따른 정점과 인접 정보 얻기
	{
		...
		{
			// 찾아야 할 인접지의 인덱스
			UINT index0 = width * (z + 0) + (x + 0); // 자기 자신
