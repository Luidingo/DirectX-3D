...
HullShader::HullShader(wstring file)
{
	...
	// 윈도우 옵션 설정 : 1. DirectX3D에서 리소스가 있을 때 사전 설정을 준수하라고 되어 있으면 그대로 따르기
	//					 2. 디버그로 설정이 되어 있어도 시킨 대로 하기

	D3DCompileFromFile(
		file.c_str(),						// 컴파일할 셰이더 코드
		nullptr,							// 추가 옵션 코드
		D3D_COMPILE_STANDARD_FILE_INCLUDE,	// 컴파일할 때 추가로 포함(참조)할 DX파일
	...
	DEVICE->CreateHullShader(
		blob->GetBufferPointer(),			// 블롭 내에 있었던(지정된) 버퍼의 메모리 포인터
